  ...
  const startSpinAnimation = async (results: CaseOpenResult[]) => {
    ...
    for (let i = 0; i < selectedNumber; i++) {
      ...
      if (targetItem && fieldControl) {
        ...
        // Для горизонтальной рулетки нужно значительно больше карточек для длинной медленной части
        const baseItemCount = selectedNumber === 1 ? 250 : 100; // Увеличиваем для более длинной прокрутки
        
        // Добавляем больше циклов случайных предметов для длинной медленной части
        const cycles = selectedNumber === 1 ? Math.floor(Math.random() * 4) + 6 : Math.floor(Math.random() * 3) + 4; // 6-9 циклов для горизонтальной, 4-6 для вертикальной
        ...
        // Добавляем больше дополнительных случайных предметов перед выигрышным для длинной медленной части
        const additionalItemsBeforeWin = selectedNumber === 1 ? 
          Math.floor(Math.random() * 50) + 40 : // 40-89 предметов для горизонтальной (увеличено для длинной медленной части)
          Math.floor(Math.random() * 25) + 20;  // 20-44 предмета для вертикальной (увеличено для длинной медленной части)
        ...
        console.log(`🎰 Поле ${i + 1}: создано ${infiniteItems.length} предметов для длинной медленной части (выигрышный на позиции ${targetIndex})`);
        
        // Добавляем больше дополнительных карточек после выигрышной для плавного визуального эффекта
        const additionalCardsAfterWin = selectedNumber === 1 ? 50 : 20; // Увеличено для более плавного эффекта
        ...
        if (selectedNumber === 1) {
          ...
          // Создаем анимацию с более длинной медленной частью для горизонтальной прокрутки
          animationPromise = fieldControl.start({
            x: finalOffset,
            transition: {
              duration: horizontalDuration,
              ease: [0.15, 0.8, 0.25, 1], // Более плавное и длинное замедление для медленной части
            }
          });
          ...
          // Создаем анимацию с более длинной медленной частью для вертикальной прокрутки
          animationPromise = fieldControl.start({
            y: finalOffset,
            transition: {
